...
function billLabelGu(billNum: number, billType: string): string {
    const ordGu = ordinaleGu(billNum);
    const typeGu = billType === 'Final' ? 'ફાઇનલ બીલ' : 'ચાલુ બીલ';
    return `${ordGu} ${typeGu}`;
}
...
export default async function BillChecklistPage({ params }: { params: Promise<{ id: string }> }) {
    await dbConnect();
        ...
        <>
            ...
            <div className="no-print bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
                <div className="flex items-center gap-3">
                    <Link href={`/bills/${id}`} className="text-gray-500 hover:text-gray-700 transition-colors">
