...
export default async function ApprovalDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const approval = await Approval.findById(id).populate('tenderId').lean();
    ...
    const sections = [
        ...
        },
        {
            ...
            fields: [
                { label: 'Linked Tender ID', value: (approval.tenderId as any)?.tenderId || 'Unknown' },
                { label: 'Package Name', value: (approval.tenderId as any)?.packageName || '-' },
                { label: 'Contractor Name', value: (approval.tenderId as any)?.contractorName || '-' },
            ]
        }
    ];
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="mb-8 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900">Technical Approval Details</h1>
                </div>
                <Link
                    href={`/approvals/${id}/edit`}
                    className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition-colors"
                >
                    <Edit2 className="w-4 h-4 mr-2" /> Edit Approval
                </Link>
            </div>

            <div className="bg-white shadow overflow-hidden sm:rounded-lg border border-gray-200">
                <div className="px-4 py-5 sm:px-6 bg-gray-50 border-b border-gray-200">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">{(approval.tenderId as any)?.packageName || 'Approval Record'}</h3>
                    <p className="mt-1 max-w-2xl text-sm text-gray-500 italic">Detailed technical approval record for the linked tender.</p>
                </div>
