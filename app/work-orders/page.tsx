                            ...
                            <table className="min-w-full divide-y divide-gray-300">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Agreement No</th>
                                        ...
                                        <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Stipulated Date</th>
                                        <th scope="col" className="relative py-3.5 pl-3 pr-4 sm:pr-6"><span className="sr-only">Edit</span></th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {workOrders.length === 0 ? (
                                        <tr><td colSpan={6} className="py-10 text-center text-sm text-gray-500">No Work Orders found.</td></tr>
                                    ) : (
                                        workOrders.map((wo: any) => (
                                            <tr key={wo._id}>
                                                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                                                    {wo.loaId ? String(wo.loaId._id).substring(0, 8) : '-'}
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{wo.agreementNo || '-'}</td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{wo.workOrderWorksheetNo || '-'}</td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{wo.workOrderDate ? new Date(wo.workOrderDate).toLocaleDateString('en-GB') : '-'}</td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{wo.stipulatedCompletionDate ? new Date(wo.stipulatedCompletionDate).toLocaleDateString('en-GB') : '-'}</td>
                                                <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                    <Link href={`/work-orders/${wo._id}/edit`} className="text-blue-600 hover:text-blue-900">Edit</Link>
                                                </td>
                                            </tr>
                                        ))
                                    )}
